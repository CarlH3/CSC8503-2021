...
bool CollisionDetection::RayCapsuleIntersection(const Ray& r, const Transform& worldTransform, const CapsuleVolume& volume, RayCollision& collision) {
	...
	Vector3 closestPointOnCentreLine = endPoint1 + (centreLine * t);

	if ((p - closestPointOnCentreLine).Length() < capsuleRadius) {
		Transform t = Transform();
		t.SetPosition(closestPointOnCentreLine);
...
bool CollisionDetection::CapsuleIntersection(const CapsuleVolume& volumeA, const Transform& worldTransformA, const CapsuleVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) {
	...
	float t = Clamp(Vector3::Dot(pointA - endPointB1, centreLineB) / Vector3::Dot(centreLineB, centreLineB), 0.0f, 1.0f);
	Vector3 closestPointOnCentreLineB = endPointB1 + (centreLineB * t);

	Vector3 centreLineA = endPointA2 - endPointA1;
	t = Clamp(Vector3::Dot(closestPointOnCentreLineB - endPointA1, centreLineA) / Vector3::Dot(centreLineA, centreLineA), 0.0f, 1.0f);
	Vector3 closestPointOnCentreLineA = endPointA1 + (centreLineA * t);

	Vector3 delta = closestPointOnCentreLineB - closestPointOnCentreLineA;
...
bool CollisionDetection::AABBCapsuleIntersection(const AABBVolume& volumeA, const Transform& worldTransformA, const CapsuleVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) {
	...
	Vector3 boxSize = volumeA.GetHalfDimensions();
	Vector3 delta = closestPointOnCentreLine - worldTransformA.GetPosition();
	Vector3 closestPointOnBox = Maths::Clamp(delta, -boxSize, boxSize);
	Vector3 localPoint = delta - closestPointOnBox;
	float distance = localPoint.Length();
...
bool CollisionDetection::SphereCapsuleIntersection(const CapsuleVolume& volumeA, const Transform& worldTransformA, const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	Vector3 delta = worldTransformB.GetPosition() - closestPointOnCentreLine;
	float deltaLength = delta.Length();
	float radii = volumeA.GetRadius() + volumeB.GetRadius();
...
bool CollisionDetection::OBBCapsuleIntersection(const OBBVolume& volumeA, const Transform& worldTransformA, const CapsuleVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) {
	...
	Quaternion invOBBOrientation = obbOrientation.Conjugate();

	Vector3 obbPos = invOBBOrientation * worldTransformA.GetPosition();
	Vector3 capsulePos = invOBBOrientation * worldTransformB.GetPosition();

	// Half height used is that of cyclinder section rather than that of whole capsule
	Vector3 capsuleUp = (worldTransformB.GetOrientation() * Vector3(0.0f, 1.0f, 0.0f)).Normalised();
	capsuleUp = (invOBBOrientation * capsuleUp).Normalised();
	...
	Vector3 delta = closestPointOnCentreLine - obbPos;
	Vector3 closestPointOnBox = Maths::Clamp(delta, -boxSize, boxSize);
	Vector3 localPoint = delta - closestPointOnBox;
	float distance = localPoint.Length();
	...
	if (distance < volumeB.GetRadius()) {
		Vector3 collisionNormal = obbOrientation * localPoint.Normalised();
		float penetration = volumeB.GetRadius() - distance;

		Vector3 localA = obbOrientation * closestPointOnBox;
		Vector3 localB = obbOrientation * (-collisionNormal * volumeB.GetRadius());
...
bool CollisionDetection::OBBIntersection(const OBBVolume& volumeA, const Transform& worldTransformA, const OBBVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) {
	...
	axes[0] = (orientationA * Vector3(1.0f, 0.0f, 0.0f)).Normalised();
	axes[1] = (orientationA * Vector3(0.0f, 1.0f, 0.0f)).Normalised();
	axes[2] = (orientationA * Vector3(0.0f, 0.0f, 1.0f)).Normalised();

	axes[3] = (orientationB * Vector3(1.0f, 0.0f, 0.0f)).Normalised();
	axes[4] = (orientationB * Vector3(0.0f, 1.0f, 0.0f)).Normalised();
	axes[5] = (orientationB * Vector3(0.0f, 0.0f, 1.0f)).Normalised();
	...
	collisionInfo.AddContactPoint(localA, localB, collisionNormal, leastPenetration);
...
bool CollisionDetection::OBBSphereIntersection(const OBBVolume& volumeA, const Transform& worldTransformA, const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) {
	...
	Vector3 obbPos = invOBBOrientation * worldTransformA.GetPosition();
	Vector3 spherePos = invOBBOrientation * worldTransformB.GetPosition();
